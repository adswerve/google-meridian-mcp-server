...
def test_worker_argv_default_prefix():
    ...
    assert argv == [sys.executable, "-m", BaseSubprocessExecutor.WORKER_MODULE, "analysis", "req", "resp"]
...
def test_child_env_backend_and_hygiene_respect_operator(monkeypatch):
    ...
    env = BaseSubprocessExecutor(env_base={"MERIDIAN_BACKEND": "jax"}).child_env({"FOO": "bar"})
...
def test_popen_redirect_never_inherits_stdout(tmp_path):
    ...
    assert kw["stdout"] is log and kw["stderr"] is log and kw["start_new_session"] is True
