...
def _input_data():
    ...
    return SimpleNamespace(
        media_channel=xr.DataArray(
            ["tv"], coords={"media_channel": ["tv"]}, dims=("media_channel",)
        ),
        media=xr.DataArray(
            [[[100.0], [120.0]]],
        ...
        rf_channel=xr.DataArray(
            ["yt"], coords={"rf_channel": ["yt"]}, dims=("rf_channel",)
        ),
        reach=xr.DataArray(
            [[[80.0], [90.0]]],
...
def test_channel_data_organic_and_non_media_types():
    rows = extract_channel_data(
        SimpleNamespace(input_data=_organic_non_media_input_data())
    )
    ...
    assert (
        email["frequency"] is None
        and email["rf_spend"] is None
        and email["value"] is None
    )
    ...
    assert (
        promo["reach"] is None
        and promo["frequency"] is None
        and promo["rf_spend"] is None
    )
