...
- **Payload is 3.2 MB** (`national-revenue`) to 16.3 MB (`geo-revenue`), far
  ...
  `ANALYSIS_MAX_RESPONSE_BYTES` default of 64 MB.
...
Worth fixing as separate work -- the plausible direction is chunking or
paginating large analysis responses rather than returning them as one SSE
event. Until then, `get_training_data` over a deployed server should be called
with a dataset or date filter; unfiltered calls on large geo models will fail.
